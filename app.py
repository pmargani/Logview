...
def run_app():
    ...
    def open_folder():
        "opens a folder and loads the sampler data"
        dir_path = QFileDialog.getExistingDirectory(window, 'Select Folder')
        if not dir_path:
            window.plot_button.setEnabled(False)
            window._sampler = None
    ...
    def loadSampler(dir_path):  
        "loads the sampler data from the given directory and updates the UI"  
        ...
        # populate the x dropdown with column names
        ...
        # populate the y_list and y2_list with column names
        for col in colnames:
    ...
    def on_plot_clicked():
        ...
        This means getting the data from the sampler and plotting it.
        """
        ...
        # getting the x-axis selection is easy
        x_col = data_selection_panel.x_dropdown.currentData()

        # but more complicated for y and y2
        ...
        try:
            ...
            def show_file_status(file_path, nfile, num_files):
                "Updates the status bar with the current file being processed."
            ...
            # get the data from our sampler, updating the status bar as we go         
        ...
        # Create the PlotData object and plot
        ...
        # Switch to graph tab - user should see the plot immediately!
